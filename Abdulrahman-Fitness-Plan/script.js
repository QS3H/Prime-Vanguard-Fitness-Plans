...
const memberConfig = {
  name: "Abdulrahman",
  startingWeight: 105.0,
  fourWeekTargetMin: 100,
  fourWeekTargetMax: 101,
  longTermTarget: 88.4,
  skeletalMuscle: 42.8,
  bodyFat: 28.5,
  bmr: 1992,
...
function updateWorkoutAProgress() {
  ...
  const completed = document.querySelectorAll(
    "#workoutA .exercise-item.completed",
  );
  ...
  document.getElementById("workoutProgressText").textContent =
    `${completed.length}/${exercises.length} exercises`;
}
...
function updateWorkoutBProgress() {
  ...
  const completed = document.querySelectorAll(
    "#workoutB .exercise-item.completed",
  );
  const progress = exercises.length
    ? (completed.length / exercises.length) * 100
    : 0;
  ...
  document.getElementById("workoutBProgressText").textContent =
    `${completed.length}/${exercises.length} exercises`;
}
...
function drawWeightChart() {
  ...
  const minWeight = Math.min(...weights, memberConfig.longTermTarget) - 2;
  const maxWeight = Math.max(...weights, memberConfig.startingWeight) + 2;
...
function applyMemberConfig() {
  ...
  if (durationEl) {
    durationEl.textContent = `Starting Weight: ${memberConfig.startingWeight.toFixed(1)} kg | 4-Week Target: ${memberConfig.fourWeekTargetMin}-${memberConfig.fourWeekTargetMax} kg | Long-Term: ${memberConfig.longTermTarget} kg`;
  }
  ...
  if (fourWeekTargetEl) {
    fourWeekTargetEl.textContent = `${memberConfig.fourWeekTargetMin}-${memberConfig.fourWeekTargetMax} kg`;
  }
}
