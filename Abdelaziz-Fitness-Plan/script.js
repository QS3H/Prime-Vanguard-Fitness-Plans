...
const memberConfig = {
  name: "Abdelazzez",
  programDuration:
    "Age 23 | Bio-Age 31 | VFI 14.9 → Target: Under 9 | Body Recomposition",
  startingWeight: 0,
  fourWeekTargetMin: 0,
  fourWeekTargetMax: 0,
  longTermTarget: 0,
...
// ===== EXERCISE TRACKER =====
function getWorkoutListId(workoutKey) {
  const map = {
    A: "workoutA",
    B: "workoutB",
    C: "workoutC",
...
function toggleExercise(checkElement, event) {
  ...
  if (workoutId === "workoutA") {
    ...
    updateWorkoutCProgress();
    saveWorkoutCProgress();
  } else if (workoutId === "workoutD") {
    updateWorkoutDProgress();
    saveWorkoutDProgress();
  }
...
function handleExerciseItemClick(item, event) {
  if (
    event.target.closest(
      'input, [contenteditable="true"], button, .weight-input-container',
    )
  ) {
    return;
  }
...
function saveWorkoutProgress(workoutKey) {
  const listId = getWorkoutListId(workoutKey);
  if (!listId) return;
  ...
  safeLocalStorageSetItem(
    `pv_workout_${workoutKey.toLowerCase()}_progress`,
    JSON.stringify(data),
  );
  showAutoSave();
}
...
function updateWorkoutProgress(listId, barId, textId) {
  const exercises = document.querySelectorAll(`#${listId} .exercise-item`);
  const completed = document.querySelectorAll(
    `#${listId} .exercise-item.completed`,
  );
  ...
  if (text) {
    text.textContent = `${completed.length}/${exercises.length} exercises`;
  }
}
...
function updateWorkoutCProgress() {
  updateWorkoutProgress(
    "workoutC",
    "workoutCProgressBar",
    "workoutCProgressText",
  );
}

function saveWorkoutCProgress() {
  saveWorkoutProgress("C");
}

function loadWorkoutCProgress() {
  loadWorkoutProgress("C", updateWorkoutCProgress);
}

function resetWorkoutCProgress() {
  resetWorkoutList("C", "workoutC", updateWorkoutCProgress, "Workout C");
}

function updateWorkoutDProgress() {
  updateWorkoutProgress(
    "workoutD",
    "workoutDProgressBar",
    "workoutDProgressText",
  );
}

function saveWorkoutDProgress() {
  saveWorkoutProgress("D");
}

function loadWorkoutDProgress() {
  loadWorkoutProgress("D", updateWorkoutDProgress);
}

function resetWorkoutDProgress() {
  resetWorkoutList("D", "workoutD", updateWorkoutDProgress, "Workout D");
}
...
function saveWaterData() {
  safeLocalStorageSetItem(
    "pv_water_count",
    JSON.stringify({ date: getTodayKey(), count: waterCount }),
  );
}
...
function logWeight() {
  ...
  let weightHistory = parseJSON(
    safeLocalStorageGetItem("pv_weight_history", "[]"),
    [],
  );
...
function updateWeightDisplay() {
  const weightHistory = parseJSON(
    safeLocalStorageGetItem("pv_weight_history", "[]"),
    [],
  );

  document.getElementById("startingWeight").textContent =
    memberConfig.startingWeight > 0
...
function drawWeightChart() {
  ...
  const chartFloor =
    memberConfig.longTermTarget > 0
      ? memberConfig.longTermTarget
      : Math.min(...weights);
  ...
  if (memberConfig.longTermTarget > 0) {
    const targetY =
      padding +
      (height - 2 * padding) *
        ((maxWeight - memberConfig.longTermTarget) / weightRange);
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      "Long-term: " + memberConfig.longTermTarget + " kg",
      width - padding,
      targetY - 5,
    );
  }
  ...
  if (data.length > 1) {
    ...
    data.forEach((point, index) => {
      ...
      const y =
        padding +
        (height - 2 * padding) * ((maxWeight - point.weight) / weightRange);
    ...
    const y =
      padding +
      (height - 2 * padding) * ((maxWeight - data[0].weight) / weightRange);
...
function updateMealProgress() {
  ...
  document.getElementById("mealProgressText").textContent =
    `${completed.length}/${meals.length} meals`;
}
...
function saveExerciseWeight(input) {
  ...
  let weightData = parseJSON(
    safeLocalStorageGetItem("pv_exercise_weights", "{}"),
    {},
  );
  ...
  const today = getTodayKey();
  const previousData = weightData[workout][exercise] || {
    weight: 0,
    date: "",
    history: [],
  };
...
function loadExerciseWeights() {
  ...
  document
    .querySelectorAll(".weight-input:not([data-no-weight])")
    .forEach((input) => {
      const workout = input.dataset.workout;
      const exercise = input.dataset.exercise;
...
function updateWeightIndicator(
  ...
  const indicator = document.getElementById(
    `weightIndicator-${workout}-${exercise}`,
  );
...
function saveExerciseName(element) {
  ...
  let editedNames = parseJSON(
    safeLocalStorageGetItem("pv_exercise_names_edited", "{}"),
    {},
  );
  ...
  safeLocalStorageSetItem(
    "pv_exercise_names_edited",
    JSON.stringify(editedNames),
  );
  showAutoSave();
}

function loadExerciseNames() {
  const editedNames = parseJSON(
    safeLocalStorageGetItem("pv_exercise_names_edited", "{}"),
    {},
  );

  document
    .querySelectorAll('.exercise-name[contenteditable="true"]')
    .forEach((element) => {
      const workout = element.dataset.workout;
      const exercise = element.dataset.exercise;
...
function saveMealItem(element) {
  ...
  let editedItems = parseJSON(
    safeLocalStorageGetItem("pv_meal_items_edited", "{}"),
    {},
  );
...
function loadMealItems() {
  const editedItems = parseJSON(
    safeLocalStorageGetItem("pv_meal_items_edited", "{}"),
    {},
  );

  document
    .querySelectorAll('.meal-items li[contenteditable="true"]')
    .forEach((element) => {
      const meal = element.dataset.meal;
      const item = element.dataset.item;
...
function captureShoppingDefaults() {
  return Array.from(document.querySelectorAll(".shopping-item-text")).map(
    (span) => span.textContent.trim(),
  );
}

function migrateShoppingTextKeys() {
  const savedTexts = parseJSON(
    safeLocalStorageGetItem("pv_shopping_list_texts", "{}"),
    {},
  );
...
function getCheckInFields() {
  const data = {};
  document
    .querySelectorAll(".progress-table input[data-field]")
    .forEach((input) => {
...
function saveCheckIn() {
  ...
  let history = parseJSON(
    safeLocalStorageGetItem("pv_checkin_history", "{}"),
    {},
  );
  history[weekKey] = {
    ...data,
...
function loadCheckIn() {
  ...
  const data =
    history[weekKey] ||
    parseJSON(safeLocalStorageGetItem("pv_checkin", "{}"), {});
...
function clearOldData() {
  if (
    confirm(
      "This will clear old check-in history (older than 8 weeks) and exercise weight history to free up storage space. Continue?",
    )
  ) {
...
function saveAllData() {
  try {
    ...
    if (error.name === "QuotaExceededError") {
      showToast(
        "Storage full. Clear old data or export progress to free space.",
      );
    } else {
...
function applyMemberConfig() {
  ...
  if (durationEl) {
    durationEl.textContent =
      memberConfig.programDuration ||
      `Starting Weight: ${memberConfig.startingWeight.toFixed(1)} kg | 4-Week Target: ${memberConfig.fourWeekTargetMin}-${memberConfig.fourWeekTargetMax} kg | Long-Term: ${memberConfig.longTermTarget} kg`;
  }
  ...
  if (fourWeekTargetEl) {
    fourWeekTargetEl.textContent =
      memberConfig.fourWeekTargetMin > 0
...
function initializeAppState() {
  ...
  loadWorkoutCProgress();
  loadWorkoutDProgress();
  initWaterTracker();
  loadCheckIn();
  loadMealProgress();
  loadExerciseWeights();
      ...
      if (this.value && (value <= 0 || value > 300)) {
        ...
        validationMsg.textContent =
          value <= 0
            ...
            : "Weight seems too high (max 300kg)";
        validationMsg.classList.add("error");
      }
    });
  }
